...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addRoleOption(opt =>
      opt.setName('ruolo')
        .setDescription('Ruolo del server da aggiungere')
        .setRequired(true))
    .addUserOption(opt =>
      opt.setName('owner1')
  ...
  async execute(interaction) {
      ...
      if (srv.roleId === ruolo.id) {
        return interaction.reply({
          embeds: [new EmbedBuilder()
            .setColor(0xFF4444)
            .setTitle('❌ Errore')
            .setDescription(`Il ruolo <@&${ruolo.id}> è già registrato come **${srv.nome}**.\nRimuovilo prima con \`/rimuovi-server\`.`)
          ],
          ephemeral: true
        });
      }
    ...
    const embed = new EmbedBuilder()
      .setColor(ruolo.color || 0x00D4FF)
      .setTitle('✅ Server Aggiunto alla Chain')
      .addFields(
        { name: '🏷️ Ruolo Server',  value: `<@&${ruolo.id}>`,  inline: true },
