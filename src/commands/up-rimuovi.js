...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('up-rimuovi')
    .setDescription('➖ Rimuovi Ultimate Points da un server della chain')
    .addStringOption(opt =>
      opt.setName('server')
        .setDescription('Nome del server a cui rimuovere i punti')
        .setRequired(true)
        .setAutocomplete(true)
    )
    .addIntegerOption(opt =>
      opt.setName('punti')
        .setDescription('Quantità di UP da rimuovere')
        .setRequired(true)
        .setMinValue(1)
        .setMaxValue(9999)
    )
    .addStringOption(opt =>
      opt.setName('motivo')
        .setDescription('Motivo della rimozione (opzionale)')
        .setRequired(false)
        .setMaxLength(200)
    ),
  ...
  async execute(interaction) {
    ...
    const nuoviPunti = db.up.scores[nomeServer];
    const colore = nuoviPunti >= 0 ? 0xFF8800 : 0xFF4444;

    const embed = new EmbedBuilder()
      .setColor(colore)
      .setTitle('➖ Ultimate Points Rimossi')
      .addFields(
        { name: '🏠 Server',        value: `**${nomeServer}**`,   inline: true },
        { name: '➖ Punti Rimossi', value: `**-${punti} UP**`,    inline: true },
        { name: '🏆 Totale',        value: `**${nuoviPunti} UP**`, inline: true },
      )
